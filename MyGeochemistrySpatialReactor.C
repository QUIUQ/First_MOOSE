...
#include "MyGeochemistrySpatialReactor.h"

registerMooseObject("GeochemistryApp", MyGeochemistrySpatialReactor);

InputParameters
MyGeochemistrySpatialReactor::sharedParams()
{
  ...
  params.addParam<std::vector<Real>>(
      ...
      {},
      "number of moles, mass or volume (depending on kinetic_species_unit) for each of the "
  ...
  params.addCoupledVar("kinetic_species_spatial_initial_value",
                       "Spacital Variaction Version of initial number of moles, mass or volume "
                       "(depending on kinetic_species_unit) for each of the "
                       "species named in kinetic_species_name");
  return params;
}

InputParameters
MyGeochemistrySpatialReactor::validParams()
{
  InputParameters params = GeochemistryReactorBase::validParams();
  params += MyGeochemistrySpatialReactor::sharedParams();
...
MyGeochemistrySpatialReactor::MyGeochemistrySpatialReactor(const InputParameters & parameters)
  : GeochemistryReactorBase(parameters),
    ...
    _nthreads(1),
    _kinetic_species_spatial_initial_value(0)
{

  // check and set the kinetic species
  if (coupledComponents("kinetic_species_spatial_initial_value") != _num_kin)
    paramError("kinetic_species_name",
               "must have the same size as kinetic_species_spatial_initial_value");
  for (unsigned i = 0; i < _num_kin; ++i)
  {
    _kinetic_species_spatial_initial_value.push_back(
        &coupledValue("kinetic_species_spatial_initial_value", i));
  }
...
void
MyGeochemistrySpatialReactor::buildMyNodeNumber()
{
  ...
  for (const auto & node : as_range(msh.local_nodes_begin(), msh.local_nodes_end()))
  {
    if (_my_node_number.count(node->id()) == 0)
      ...
      mooseError(
          "MyGeochemistrySpatialReactor: something wrong with node numbering in buildMyNodeNumber");
...
void
MyGeochemistrySpatialReactor::initialSetup()
{
...
void
MyGeochemistrySpatialReactor::initialize()
{
...
void
MyGeochemistrySpatialReactor::execute()
{
  for (unsigned e = 0; e < _num_my_nodes; ++e)
  {

    std::vector<Real> kin_values;
    kin_values.reserve(_kinetic_species_spatial_initial_value.size());

    for (unsigned j = 0; j < _kinetic_species_spatial_initial_value.size(); ++j)
    {
      //  GeochemicalSystem
      kin_values.push_back((*_kinetic_species_spatial_initial_value[j])[e]);
    }

    //  GeochemicalSystem
    _egs_at_node.push_back(
        GeochemicalSystem(_mgd_at_node[e],
                          _gac,
                          _is,
                          _swapper,
                          getParam<std::vector<std::string>>("swap_out_of_basis"),
...
void
MyGeochemistrySpatialReactor::threadJoin(const UserObject & uo)
{
  _nthreads += 1;
  const auto & gsr = static_cast<const MyGeochemistrySpatialReactor &>(uo);
  for (unsigned i = 0; i < _num_my_nodes; ++i)
  {
...
void
MyGeochemistrySpatialReactor::finalize()
{
  ...
  {
    std::vector<MyGeochemistrySpatialReactor *> objects;
    ...
    mooseAssert(objects.size() == 1,
                "MyGeochemistrySpatialReactor::finalize() failed to obtain a single thread copy of "
                "the MyGeochemistrySpatialReactor");
...
void
MyGeochemistrySpatialReactor::meshChanged()
{
  mooseError("MyGeochemistrySpatialReactor cannot yet handle adaptive meshing");
  /*
...
MyGeochemistrySpatialReactor::getGeochemicalSystem(dof_id_type node_id) const
{
  if (_my_node_number.count(node_id) != 1)
    mooseError("MyGeochemistrySpatialReactor does not know about node ", node_id);
  return _egs_at_node[_my_node_number.at(node_id)];
}

const DenseVector<Real> &
MyGeochemistrySpatialReactor::getMoleAdditions(dof_id_type node_id) const
{
  if (_my_node_number.count(node_id) != 1)
    mooseError("MyGeochemistrySpatialReactor does not know about node ", node_id);
...
MyGeochemistrySpatialReactor::getSolverOutput(dof_id_type node_id) const
{
  if (_my_node_number.count(node_id) != 1)
    mooseError("MyGeochemistrySpatialReactor does not know about node ", node_id);
...
unsigned
MyGeochemistrySpatialReactor::getSolverIterations(dof_id_type node_id) const
{
  if (_my_node_number.count(node_id) != 1)
    mooseError("MyGeochemistrySpatialReactor does not know about node ", node_id);
...
Real
MyGeochemistrySpatialReactor::getSolverResidual(dof_id_type node_id) const
{
  if (_my_node_number.count(node_id) != 1)
    mooseError("MyGeochemistrySpatialReactor does not know about node ", node_id);
...
Real
MyGeochemistrySpatialReactor::getMolesDumped(dof_id_type /*node_id*/,
